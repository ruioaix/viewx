$(document).ready(function() {
    ...
    $('#text').remove();
    $('#canvas_container').prepend('<div id ="text"> <h3> Found {{ count($cheatings) }} cheating</h3> <ul> @foreach ($cheatings as $cht) <li> <a target="_blank" href="{{ action('TaskController@czh', ["$cht"]) }}">{{ $cht }} </a></li> @endforeach </ul> </div>');
